...
pub struct MemoryUse {
    ...
    /// Bytes used by the application according to our own memory allocator's
    /// accounting.
    ///
    /// This can be smaller than [`Self::resident`] because our memory allocator
    /// may not return all the memory we free to the OS.
    ///
...
impl MemoryUse {
    ...
    ///
    /// This is either [`Self::counted`] if it's available, otherwise fallbacks
    /// to [`Self::resident`] if that's available, otherwise `None`.
    #[inline]
    pub fn used(&self) -> Option<usize> {
        self.counted.or(self.resident)
    }

    pub fn print_memory_usage_info(&self, verbosity: log::Level) {
        if let Some(mem_resident) = self.resident {
            log!(verbosity, "MB resident total: {}", (mem_resident / (1024 * 1024)));
        }
        if let Some(mem_counted) = self.counted {
...
fn bytes_resident() -> Option<usize> {
...
fn bytes_resident() -> Option<usize> {
    ...
    // None
    //approximation through the min max pointer
    Some(crate::accounting_allocator::bytes_allocated_approx())
}
