...
use crate::{
    ...
    bundle::{DynamicBundleClone, DynamicClone},
    Layout,
};
...
use crate::{
    ...
    Component, ComponentRef, ComponentRefShared, DynamicBundle,
};

//in order to avoid using the entityBuilder as a component we deimplement send
// and sync for it so it cannot be used as a component. this is a very hacky
// thing and we should think of a better way to avoid it but since hecs doesn't
// support nested bundles, it's kinda tricky. use negative_impl::negative_impl;
// #[negative_impl]
...
impl EntityBuilder {
    ...
    pub fn build(&mut self) -> BuiltEntity<'_> {
        ...
        self.inner.ids.extend(self.inner.info.iter().map(|x| x.0.id()));
        BuiltEntity { builder: &mut self.inner }
    }
...
impl EntityBuilderClone {
    ...
    pub fn add<T: Component + Clone>(&mut self, mut component: T) -> &mut Self {
        unsafe {
            self.inner
                .add((&mut component as *mut T).cast(), TypeInfo::of::<T>(), DynamicClone::new::<T>());
        }
...
impl<M> Common<M> {
    ...
    unsafe fn grow(min_size: usize, cursor: usize, align: usize, storage: NonNull<u8>) -> (NonNull<u8>, Layout) {
        let layout = Layout::from_size_align(min_size.next_power_of_two().max(64), align).unwrap();
        let core_layout = core::alloc::Layout::from_size_align(layout.size(), layout.align()).unwrap();
            ...
            REntry::Vacant(vacant) => {
                ...
                if end > self.layout.size() || ty.layout().align() > self.layout.align() {
                    ...
                    if self.layout.size() != 0 {
                        let core_layout = core::alloc::Layout::from_size_align(self.layout.size(), self.layout.align()).unwrap();
        ...
        unsafe {
            ...
            for &(_, offset, ref clone) in &self.info {
                (clone.func)(self.storage.as_ptr().add(offset), &mut |src, ty| {
                    result.storage.as_ptr().add(offset).copy_from_nonoverlapping(src, ty.layout().size());
                });
            }
            result
        }
