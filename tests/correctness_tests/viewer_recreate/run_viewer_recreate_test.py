...
# This checks if we can recreate the viewer without a memory leak.
# The memory leak can happen if rust destroys the wgpu device before all the other gpu buffers
...
def test_viewer_recreate():
    ...
    while True:
        print("step", step)
        _ = ViewerHeadless(256, 256, path_config)  # Resolution of the rendering window
        step += 1
        if step == 500:
