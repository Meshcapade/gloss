...
use crate::{
    alloc::boxed::Box,
    archetype::{Archetype, StableTypeIdMap, TypeInfo},
    entities::{Entities, EntityMeta, Location, ReserveEntitiesIterator},
    query::assert_borrow,
...
impl World {
    ...
    pub fn spawn_at(&mut self, handle: Entity, components: impl DynamicBundle) {
        ...
        if let Some(loc) = loc {
            if let Some(moved) = unsafe { self.archetypes.archetypes[loc.archetype as usize].remove(loc.index, true) } {
        ...
        let archetype_id = match components.key() {
            Some(k) => {
                ...
                *self
                    .bundle_to_archetype
                    .entry(k)
                    .or_insert_with(|| components.with_ids(|ids| archetypes.get(ids, || RVec::from(components.type_info()))))
            }
            None => components.with_ids(|ids| self.archetypes.get(ids, || RVec::from(components.type_info()))),
        };
        ...
        I::Item: Bundle + 'static,
    {
        ...
        let archetype_id = self.reserve_inner::<I::Item>(u32::try_from(upper.unwrap_or(lower)).expect("iterator too large"));
    ...
    pub fn despawn(&mut self, entity: Entity) -> Result<(), NoSuchEntity> {
        ...
        for ty in archetype.types() {
            let removed_entities = self.removed_components.entry(ty.id()).or_insert_with(RVec::new);
    ...
    fn reserve_inner<T: Bundle + 'static>(&mut self, additional: u32) -> u32 {
        ...
        let archetype_id = *self
            ...
            .or_insert_with(|| T::with_static_ids(|ids| archetypes.get(ids, || T::with_static_type_info(|info| RVec::from(info.to_vec())))));
    ...
    pub fn clear(&mut self) {
        for archetype in &self.archetypes.archetypes {
            for ty in archetype.types() {
                let archetype_entities: Vec<Entity> = archetype.ids().iter().map(|id| unsafe { self.find_entity_from_id(*id) }).collect();
                let removed_entities = self.removed_components.entry(ty.id()).or_insert_with(RVec::new);
    ...
    pub fn query_one<Q: Query>(&self, entity: Entity) -> Result<QueryOne<'_, Q>, NoSuchEntity> {
        ...
        Ok(unsafe { QueryOne::new(&self.archetypes.archetypes[loc.archetype as usize], loc.index) })
    }
    ...
    pub fn query_one_mut<Q: Query>(&mut self, entity: Entity) -> Result<QueryItem<'_, Q>, QueryOneError> {
        assert_borrow::<Q>();
    ...
    pub fn get<'a, T: ComponentRef<'a>>(&'a self, entity: Entity) -> Result<T::Ref, ComponentError> {
        Ok(self.entity(entity)?.get::<T>().ok_or_else(MissingComponent::new::<T::Component>)?)
    }
    ...
    pub fn entity(&self, entity: Entity) -> Result<EntityRef<'_>, NoSuchEntity> {
        let loc = self.entities.get(entity)?;
        unsafe { Ok(EntityRef::new(&self.archetypes.archetypes[loc.archetype as usize], entity, loc.index)) }
    }
    ...
    /// in `loc.archetype`.
    fn insert_inner(&mut self, entity: Entity, components: impl DynamicBundle, graph_origin: u32, loc: Location) {
        let target_storage;
        ...
        unsafe {
            ...
            for &ty in &target.replaced {
                let ptr = source_arch.get_dynamic(ty.id(), ty.layout().size(), loc.index).unwrap();
                ty.drop(ptr.as_ptr());
            }
            ...
            let (source_arch, target_arch) = index2(&mut self.archetypes.archetypes, loc.archetype as usize, target.index as usize);
            ...
            components.put(|ptr, ty| {
                ...
                target_arch.put_dynamic(ptr, ty.id(), ty.layout().size(), target_index, !had_component, had_component);
            });

            // Move the components we're keeping
            for &ty in &target.retained {
                let src = source_arch.get_dynamic(ty.id(), ty.layout().size(), loc.index).unwrap();
                ...
                target_arch.put_dynamic(src.as_ptr(), ty.id(), ty.layout().size(), target_index, added, mutated);
            }
    ...
    pub fn remove<T: Bundle + 'static>(&mut self, entity: Entity) -> Result<T, ComponentError> {
        ...
        let bundle = unsafe { T::get(|ty| source_arch.get_dynamic(ty.id(), ty.layout().size(), old_index))? };

        // Find the target archetype ID
        let target = Self::remove_target::<T>(&mut self.archetypes, &mut self.remove_edges, loc.archetype);
        ...
        if loc.archetype != target {
            ...
            // new archetype
            let (source_arch, target_arch) = index2(&mut self.archetypes.archetypes, loc.archetype as usize, target as usize);
            ...
            if let Some(moved) = unsafe {
                source_arch.move_to(old_index, |src, ty, size, is_added, is_mutated| {
                    ...
                    if let Some(dst) = target_arch.get_dynamic(ty, size, target_index) {
                        ...
                        *target_arch.get_added(state).as_ptr().add(target_index as usize) = is_added;
                        *target_arch.get_mutated(state).as_ptr().add(target_index as usize) = is_mutated;
                    } else {
                        let removed_entities = removed_components.entry(ty).or_insert_with(RVec::new);
                        removed_entities.push(entity);
                    }
                })
            } {
    ...
    fn remove_target<T: Bundle + 'static>(archetypes: &mut ArchetypeSet, remove_edges: &mut IndexStableTypeIdMap<u32>, old_archetype: u32) -> u32 {
        match remove_edges.entry(Tuple2(old_archetype, StableTypeId::of::<T>())) {
            ...
            REntry::Vacant(entry) => {
                ...
                let elements = info.iter().map(super::archetype::TypeInfo::id).collect::<Box<_>>();
    ...
    pub fn exchange<S: Bundle + 'static, T: DynamicBundle>(&mut self, entity: Entity, components: T) -> Result<S, ComponentError> {
        self.flush();
        ...
        let bundle = unsafe { S::get(|ty| source_arch.get_dynamic(ty.id(), ty.layout().size(), loc.index))? };

        // Find the intermediate archetype ID
        let intermediate = Self::remove_target::<S>(&mut self.archetypes, &mut self.remove_edges, loc.archetype);
    ...
    pub fn exchange_one<S: Component, T: Component>(&mut self, entity: Entity, component: T) -> Result<S, ComponentError> {
        self.exchange::<(S,), (T,)>(entity, (component,)).map(|(x,)| x)
    }
    ...
    pub unsafe fn get_unchecked<'a, T: ComponentRef<'a>>(&'a self, entity: Entity) -> Result<T, ComponentError> {
        let loc = self.entities.get(entity)?;
        let archetype = &self.archetypes.archetypes[loc.archetype as usize];
        let state = archetype.get_state::<T::Component>().ok_or_else(MissingComponent::new::<T::Component>)?;
        Ok(T::from_raw(archetype.get_base::<T::Component>(state).as_ptr().add(loc.index as usize)))
    }
    ...
    pub fn flush(&mut self) {
        ...
        self.entities.flush(|id, location| location.index = unsafe { arch.allocate(id) });
    }
    ...
    pub fn take(&mut self, entity: Entity) -> Result<TakenEntity<'_>, NoSuchEntity> {
        ...
        let loc = self.entities.get(entity)?;
        let archetype = &mut self.archetypes.archetypes[loc.archetype as usize];
        unsafe { Ok(TakenEntity::new(&mut self.entities, entity, archetype, loc.index)) }
    }
    ...
    /// Clears each entity's tracker state. For example, each entity's component
    /// "mutated" state will be reset to `false`.
...
impl fmt::Display for ComponentError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        use ComponentError::{MissingComponent, NoSuchEntity};
        match *self {
...
impl fmt::Display for QueryOneError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        use QueryOneError::{NoSuchEntity, Unsatisfied};
        match *self {
            NoSuchEntity => f.write_str("no such entity"),
    ...
    I::Item: Bundle,
{
    ...
    fn next(&mut self) -> Option<Entity> {
        ...
        unsafe {
            components.put(|ptr, ty| {
                self.archetype.put_dynamic(ptr, ty.id(), ty.layout().size(), index, true, false);
            });
        }
        self.entities.meta[entity.id as usize].location = Location {
...
impl ArchetypeSet {
    ...
    fn get<T: Borrow<[StableTypeId]> + Into<Box<[StableTypeId]>>>(&mut self, components: T, info: impl FnOnce() -> RVec<TypeInfo>) -> u32 {
    ...
    // fn insert(&mut self, components: RSlice<'_, StableTypeId>, info:
    // RVec<TypeInfo>) -> u32 {
    ...
    fn insert_batch(&mut self, archetype: Archetype) -> (u32, u32) {
        let ids = archetype.types().iter().map(super::archetype::TypeInfo::id).collect::<Box<_>>();
    ...
    fn get_insert_target(&mut self, src: u32, components: &impl DynamicBundle) -> InsertTarget {
        ...
        let elements: Box<[StableTypeId]> = info.iter().map(super::archetype::TypeInfo::id).collect();
        // let r_elements: RSlice<StableTypeId> = RSlice::from(&*elements);
        let r_info = RVec::from(info);
        let index = self.get(elements, move || r_info);
        InsertTarget { replaced, retained, index }
    }
}
...
type IndexStableTypeIdMap<V> = RHashMap<Tuple2<u32, StableTypeId>, V, BuildHasherDefault<IndexStableTypeIdHasher>>;
