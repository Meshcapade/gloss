...
// This is done to avoid changes to the hecs code
...
use crate::{Archetype, ColumnBatch, ColumnBatchBuilder, ColumnBatchType, Component, Entity, World};
...
/// use gloss_hecs::{serialize::column::*, *};
...
///             .component_types()
///             .filter(|&t| {
///                 t == StableTypeId::of::<Position>() || t == StableTypeId::of::<Velocity>()
///             })
///             .count()
///     }
///
...
pub trait SerializeContext {
    ...
    fn serialize_component_ids<S: SerializeTuple>(&mut self, archetype: &Archetype, out: S) -> Result<S::Ok, S::Error>;
    ...
    fn serialize_components<S: SerializeTuple>(&mut self, archetype: &Archetype, out: S) -> Result<S::Ok, S::Error>;
}
    ...
    C: SerializeContext,
{
        ...
        C: SerializeContext,
    {
            ...
            S: Serializer,
        {
            ...
            self.ctx.borrow_mut().serialize_component_ids(self.archetype, tuple)
        }
    }
    ...
    let mut seq = serializer.serialize_seq(Some(world.archetypes().filter(|x| !x.is_empty()).count()))?;
...
pub fn deserialize_column<'de, T, A>(entity_count: u32, seq: &mut A, out: &mut ColumnBatchBuilder) -> Result<(), A::Error>
where
    T: Component + Deserialize<'de>,
    A: SeqAccess<'de>,
{
    seq.next_element_seed(DeserializeColumn::<T>::new(entity_count, out))?
        .ok_or_else(|| de::Error::invalid_value(Unexpected::Other("end of components"), &"a column of components"))
}
    ...
    T: Component + Deserialize<'de>,
{
    ...
    fn expecting(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
        write!(formatter, "a set of {} {} values", self.entity_count, type_name::<T>())
    }
        ...
        A: SeqAccess<'de>,
    {
        ...
        while let Some(component) = seq.next_element()? {
            if out.push(component).is_err() {
                return Err(de::Error::invalid_value(Unexpected::Other("extra component"), &self));
            }
        }
    ...
    C: DeserializeContext,
{
        ...
        A: SeqAccess<'de>,
    {
        ...
        while let Some(bundle) = seq.next_element_seed(DeserializeArchetype(self.0, &mut entities))? {
    ...
    C: DeserializeContext,
{
    ...
    where
        A: SeqAccess<'de>,
    {
        let entity_count = seq.next_element::<u32>()?.ok_or_else(|| de::Error::invalid_length(0, &self))?;
        let component_count = seq.next_element::<u32>()?.ok_or_else(|| de::Error::invalid_length(1, &self))?;
        ...
        batch
            .build()
            .map_err(|_| de::Error::invalid_value(Unexpected::Other("incomplete archetype"), &"a complete archetype"))
    }
}
    ...
    C: DeserializeContext,
{
        ...
        A: SeqAccess<'de>,
    {
        ...
        self.ctx.deserialize_components(self.entity_count, seq, self.out)
    }
}
...
mod tests {
    ...
    impl PartialEq for SerWorld {
        ...
        fn eq(&self, other: &Self) -> bool {
            ...
            for (x, y) in self.0.iter().zip(other.0.iter()) {
                if x.entity() != y.entity() || !same_components::<Position>(&x, &y) || !same_components::<Velocity>(&x, &y) {
                    return false;
                }
            }
            true
        ...
        fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
            f.debug_map()
                .entries(
                    self.0
                        .iter()
                        .map(|e| (e.entity(), (e.get::<&Position>().map(|x| *x), e.get::<&Velocity>().map(|x| *x)))),
                )
                .finish()
        }
    }
    ...
    mod helpers {
        ...
        pub fn serialize<S: Serializer>(x: &World, s: S) -> Result<S::Ok, S::Error> {
            crate::serialize::column::serialize(x, &mut Context { components: Vec::new() }, s)
        }
        pub fn deserialize<'de, D: Deserializer<'de>>(d: D) -> Result<World, D::Error> {
            crate::serialize::column::deserialize(&mut Context { components: Vec::new() }, d)
        }
    }
