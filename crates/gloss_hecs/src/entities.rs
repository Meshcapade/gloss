...
///
/// Obtained from `World::spawn`. Can be stored to refer to an entity in the
/// future.
///
/// Enable the `serde` feature on the crate to make this `Serialize`able. Some
/// applications may be able to save space by only serializing the output of
/// `Entity::id`.
#[derive(Clone, Copy, Hash, Eq, Ord, PartialEq, PartialOrd)]
#[repr(C)]
#[cfg_attr(not(target_arch = "wasm32"), derive(StableAbi))]
#[cfg_attr(not(target_arch = "wasm32"), sabi(missing_field(panic)))]
pub struct Entity {
...
impl Entity {
    /// An [`Entity`] that does not necessarily correspond to data in any
    /// `World`
    ///
    ...
    ///
    /// Useful for storing entity IDs externally, or in conjunction with
    ...
    /// bitpattern is valid, else `None`
    ///
    /// Useful for storing entity IDs externally, or in conjunction with
    /// `Entity::to_bits` and `World::spawn_at` for easy serialization.
...
pub(crate) struct Entities {
    ...
    //
    // - The `freelist` IDs, previously freed by `free()`. These IDs are available to any of `alloc()`, `reserve_entity()` or `reserve_entities()`.
    //   Allocation will always prefer these over brand new IDs.
    //
    // - The `reserved` list of IDs that were once in the freelist, but got reserved by `reserve_entities` or `reserve_entity()`. They are now
    //   waiting for `flush()` to make them fully allocated.
    //
    // - The count of new IDs that do not yet exist in `self.meta()`, but which we have handed out and reserved. `flush()` will allocate room for
    //   them in `self.meta()`.
    //
...
impl Entities {
    ...
    fn verify_flushed(&mut self) {
        debug_assert!(!self.needs_flush(), "flush() needs to be called before this operation is legal");
    }
    ...
    /// Allocate and set locations for many entity IDs laid out contiguously in
    /// an archetype
    ///
    ...
    pub fn alloc_many(&mut self, n: u32, archetype: u32, mut first_index: u32) -> AllocManyState {
        ...
        assert!((self.meta.len() + fresh as usize) < u32::MAX as usize, "too many entities");
        ...
        self.meta.extend((first_index..(first_index + fresh)).map(|index| EntityMeta {
            generation: NonZeroU32::new(1).unwrap(),
    ...
    ///
    /// This is an awkward separate function to avoid borrowck issues in
    ...
    pub fn alloc_at(&mut self, entity: Entity) -> Option<Location> {
        ...
        let loc = if entity.id as usize >= self.meta.len() {
            ...
            Some(mem::replace(&mut self.meta[entity.id as usize].location, EntityMeta::EMPTY.location))
        };
    ...
    pub fn free(&mut self, entity: Entity) -> Result<Location, NoSuchEntity> {
        ...
        meta.generation = NonZeroU32::new(u32::from(meta.generation).wrapping_add(1)).unwrap_or_else(|| NonZeroU32::new(1).unwrap());
    ...
    pub fn contains(&self, entity: Entity) -> bool {
        ...
        //                 || self.pending[self.free_cursor.load(Ordering::Relaxed).max(0) as usize..].contains(&entity.id))
        //     }
        //     None => {
        //         // Check if this could have been obtained from `reserve_entity`
        //         let free = self.free_cursor.load(Ordering::Relaxed);
        //         entity.generation.get() == 1 && free < 0 && (entity.id as isize) <
        // (free.abs() + self.meta.len() as isize)     }
        // }
        if let Some(meta) = self.meta.get(entity.id as usize) {
            meta.generation == entity.generation
                && (meta.location.index != u32::MAX || self.pending[self.free_cursor.load(Ordering::Relaxed).max(0) as usize..].contains(&entity.id))
        } else {
            // Check if this could have been obtained from `reserve_entity`
            let free = self.free_cursor.load(Ordering::Relaxed);
            entity.generation.get() == 1 && free < 0 && (entity.id as isize) < (free.abs() + self.meta.len() as isize)
        }
    }
    ...
    /// Returns `Ok(Location { archetype: 0, index: undefined })` for pending
    /// entities
    pub fn get(&self, entity: Entity) -> Result<Location, NoSuchEntity> {
        if self.meta.len() <= entity.id as usize {
            // Check if this could have been obtained from `reserve_entity`
            let free = self.free_cursor.load(Ordering::Relaxed);
            if entity.generation.get() == 1 && free < 0 && (entity.id as isize) < (free.abs() + self.meta.len() as isize) {
                return Ok(Location {
                    archetype: 0,
                    index: u32::MAX,
                });
...
mod tests {
    ...
    fn entity_bits_roundtrip() {
        let e = Entity {
            generation: NonZeroU32::new(0xDEAD_BEEF).unwrap(),
            id: 0xBAAD_F00D,
        };
    ...
    fn alloc_and_free() {
        ...
        let mut id_to_gen: HashMap<u32, u32> = HashMap::default();
        let mut free_set: HashSet<u32> = HashSet::default();
        let mut len = 0;
        ...
        for _ in 0..100 {
            ...
            if alloc || first_unused == 0 {
                ...
                let entity = Entity {
                    id,
                    generation: NonZeroU32::new(generation.unwrap_or_else(|| NonZeroU32::new(1).unwrap().get())).unwrap(),
                };
    ...
    fn alloc_at() {
        ...
        // Allocating an entity at an unused location should not cause a location to be
        // returned.
