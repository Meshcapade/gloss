use crate::{alloc::collections::BinaryHeap, stabletypeid::StableTypeId};
...
impl ColumnBatchType {
    ...
    pub fn into_batch(self, size: u32) -> ColumnBatchBuilder {
        ...
        types.dedup();
        let fill = StableTypeIdMap::with_capacity_and_hasher(types.len(), BuildHasherDefault::default());
...
impl ColumnBatchBuilder {
    ...
    pub fn writer<T: Component>(&mut self) -> Option<BatchWriter<'_, T>> {
        ...
        Some(BatchWriter {
            ...
            storage: unsafe { slice::from_raw_parts_mut(base.as_ptr().cast(), self.target_fill as usize).iter_mut() },
        })
    }
