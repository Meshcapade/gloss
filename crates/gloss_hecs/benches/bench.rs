...
use gloss_hecs::{Bundle, CommandBuffer, EntityBuilder, EntityBuilderClone, PreparedQuery, World};
...
fn remove(b: &mut Bencher) {
    ...
    b.iter(|| {
        ...
        let entities = world.spawn_batch((0..1_000).map(|_| (Position(0.0), Velocity(0.0)))).collect::<Vec<_>>();
        for e in entities {
...
fn insert(b: &mut Bencher) {
    ...
    b.iter(|| {
        // This really shouldn't be counted as part of the benchmark, but bencher
        // doesn't seem to support that.
        let entities = world.spawn_batch((0..1_000).map(|_| (Position(0.0),))).collect::<Vec<_>>();
        for e in entities {
