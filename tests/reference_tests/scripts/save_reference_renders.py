...
def save_silhouette_and_depth(extrinsics, intrinsics, width, height, camera_id, scene):
    ...
    reference_dir_silhouette = osp.join(
        osp.dirname(osp.realpath(__file__)), "../silhouette_test/references"
    )
    save_path_silhouette = osp.join(reference_dir_silhouette, str(camera_id) + ".png")
    cv2.imwrite(save_path_silhouette, bg_fg_arr)

    reference_dir_depth = osp.join(
        osp.dirname(osp.realpath(__file__)), "../depth_test/references"
    )
...
if __name__ == "__main__":
    ...
    with Progress() as progress:
        ...
        for camera in cameras:
            ...
            save_silhouette_and_depth(
                extrinsics, intrinsics, width, height, camera_id, scene
            )
