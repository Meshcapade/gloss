...
pub fn derive(input: DeriveInput) -> Result<TokenStream2> {
    ...
    let data = match input.data {
        syn::Data::Struct(s) => s,
        _ => return Err(Error::new_spanned(ident, "derive(DynamicBundleClone) does not support enums or unions")),
    };
...
fn gen_dynamic_bundle_impl(ident: &syn::Ident, generics: &syn::Generics, field_members: &[syn::Member], tys: &[&syn::Type]) -> TokenStream2 {
    ...
    quote! {
        unsafe impl #impl_generics ::gloss_hecs::DynamicBundleClone for #ident #ty_generics #where_clause {
            #[allow(clippy::forget_copy)]
            unsafe fn put_with_clone(mut self, mut f: impl ::std::ops::FnMut(*mut u8, ::gloss_hecs::TypeInfo, ::gloss_hecs::DynamicClone)) {
                #(
                    f(
                        (&mut self.#field_members as *mut #tys).cast::<u8>(),
                        ::gloss_hecs::TypeInfo::of::<#tys>(),
                        ::gloss_hecs::DynamicClone::new::<#tys>()
                    );
...
fn add_additional_bounds_to_generic_params(mut generics: syn::Generics) -> syn::Generics {
    for tp in generics.type_params_mut() {
        tp.bounds.push(syn::TypeParamBound::Trait(make_component_trait_bound()))
    }
    generics
}
