...
def test_depth():
    ...
    assert osp.exists(
        config_path
    ), f"Config for the test ({config_file}) does not seem to exist!\n \
    ...
    viewer = gloss.ViewerHeadless(
        2048, 2048, config_path
    )  # Dummy height width, will be overwritten
    ...
    scene.get_or_create_entity("body").insert_builder(
        builders.build_from_file(obj_path)
    )
    ...
    with Progress() as progress:
        task = progress.add_task(
            "Testing gloss against reference depths", total=len(cameras)
        )
        for camera in cameras:
            ...
            gloss_depth_np_remapped = np.squeeze(
                gloss_depth.depth_linearize(device, queue, znear, zfar)
            )
            ...
            path_to_reference = osp.join(
                root_path, "references", str(camera_id) + ".tiff"
            )
            path_to_reference_silhouette = osp.join(
                root_path, "../silhouette_test", "references", str(camera_id) + ".png"
            )
            reference_depth = cv2.imread(path_to_reference, cv2.IMREAD_UNCHANGED)
            reference_silhouette = (
                cv2.imread(path_to_reference_silhouette, cv2.IMREAD_UNCHANGED) / 255
            ).astype(np.uint)
            silhouette_mask = (
                np.bitwise_and(reference_silhouette, gloss_silhouette) == 1
            )

            image_difference = (
                gloss_depth_np_remapped[silhouette_mask]
                - reference_depth[silhouette_mask]
            )
            ...
            assert (
                depth_error < 1e-2
            ), "[TEST FAILED] Image difference is larger than 10^-2"
