...
///
/// Once rust offers generic associated types, this will be moved into
/// [`Query`].
...
impl<'a, T: Component> Mut<'a, T> {
    /// Creates a new mutable reference to a component. This is unsafe because
    /// the index bounds are not checked.
    ///
    /// # Safety
...
impl<'q, Q: Query> BatchedIter<'q, Q> {
    ...
    /// safety, either with dynamic borrow checks or by representing
    /// exclusive access to the `World`.
    unsafe fn new(meta: &'q [EntityMeta], archetypes: SliceIter<'q, Archetype>, batch_size: u32) -> Self {
        Self {
            _marker: PhantomData,
            meta,
            archetypes,
...
impl<'q, Q: Query> Iterator for BatchedIter<'q, Q> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            ...
            self.archetypes = archetypes;
            debug_assert_eq!(self.batch, 0, "query fetch should always reject at the first batch or not at all");
            // continue;
        }
    }
}
...
impl<Q: Query> PreparedQuery<Q> {
    ...
    pub fn query_mut<'q>(&'q mut self, world: &'q mut World) -> PreparedQueryIter<'q, Q> {
        ...
        let state: &'q [(usize, <Q::Fetch as Fetch<'q>>::State)] = unsafe { mem::transmute(&*self.state) };
...
impl<'q, Q: Query> PreparedQueryBorrow<'q, Q> {
    ...
    pub fn iter<'i>(&'i mut self) -> PreparedQueryIter<'i, Q> {
        let state: &'i [(usize, <Q::Fetch as Fetch<'i>>::State)] = unsafe { mem::transmute(self.state) };
...
impl<'q, Q: Query> PreparedQueryIter<'q, Q> {
    ...
    /// safety, either with dynamic borrow checks or by representing
    /// exclusive access to the `World`.
    unsafe fn new(meta: &'q [EntityMeta], archetypes: &'q [Archetype], state: SliceIter<'q, (usize, <Q::Fetch as Fetch<'q>>::State)>) -> Self {
        Self {
            meta,
            archetypes,
            state,
...
impl<Q: Query> ExactSizeIterator for PreparedQueryIter<'_, Q> {
    fn len(&self) -> usize {
        self.state.clone().map(|(idx, _)| self.archetypes[*idx].len() as usize).sum::<usize>() + self.iter.remaining()
    }
}
...
impl<'q, Q: Query> View<'q, Q> {
    ...
    unsafe fn new(meta: &'q [EntityMeta], archetypes: &'q [Archetype]) -> Self {
        let fetch = archetypes
            .iter()
            .map(|archetype| Q::Fetch::prepare(archetype).map(|state| Q::Fetch::execute(archetype, state)))
            .collect();
    ...
    /// ```
    pub fn get_mut_n<const N: usize>(&mut self, entities: [Entity; N]) -> [Option<QueryItem<'_, Q>>; N] {
