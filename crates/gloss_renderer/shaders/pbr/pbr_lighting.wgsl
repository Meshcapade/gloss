...
#import ../utils/constants.wgsl as Constants
...
fn Fd_Burley(roughness: f32, NoV: f32, NoL: f32, LoH: f32) -> f32 {
    ...
    return lightScatter * viewScatter * (1.0 / Constants::PI);
}
...
fn point_light(
    ...
    let light_to_frag = light.pos_world - world_position.xyz;
    let distance_square = dot(light_to_frag, light_to_frag);
    let rangeAttenuation = getDistanceAttenuation(distance_square, light.inverse_square_range);
    ...
    return ((diffuse + specular_light) * light.intensity * light.color.rgb) * (rangeAttenuation * NoL);
    // return ((diffuse ) * light.intensity * light.color.rgb) * (rangeAttenuation * NoL);
}
...
fn spot_light(
    ...
    let cosOuter = cos(light.outer_angle);
    let spotScale = 1.0 / max(cos(light.inner_angle) - cosOuter, 1e-4);
    ...
    // }
    let spot_dir = light.lookat_dir_world;
    let light_to_frag = light.pos_world - world_position.xyz;
    ...
    let attenuation = saturate(cd * spotScale + spotOffset);
