...
def test_silhouette():
    ...
    with Progress() as progress:
        task = progress.add_task(
            "Testing gloss against reference silhouettes", total=len(cameras)
        )
        for camera in cameras:
            ...
            path_to_reference = osp.join(
                root_path, "references", str(camera_id) + ".png"
            )
            ...
            percentage = 100 * (
                num_different_pixels / (camera["height"] * camera["width"])
            )
            all_num_different_pixels.append(f"{percentage:.4f}%")

            assert (
                percentage < 0.1
            ), f"[TEST FAILED] Image difference is larger than 0.1%, {int(num_different_pixels)} different pixels!"
            progress.update(task, advance=1)

    print("[ALL TESTS SUCCEEDED] with an image difference of less than 0.1%")
    print(
        "Percentage of the image with pixels that are not exactly the same - ",
        all_num_different_pixels,
    )
