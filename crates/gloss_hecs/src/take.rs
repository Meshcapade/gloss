...
use crate::{entities::Entities, stabletypeid::StableTypeId, Archetype, DynamicBundle, Entity, TypeInfo};
...
impl<'a> TakenEntity<'a> {
    ...
    pub(crate) unsafe fn new(entities: &'a mut Entities, entity: Entity, archetype: &'a mut Archetype, index: u32) -> Self {
        Self {
            entities,
            entity,
            archetype,
...
unsafe impl<'a> DynamicBundle for TakenEntity<'a> {
    ...
    unsafe fn put(mut self, mut f: impl FnMut(*mut u8, TypeInfo)) {
        ...
        for &ty in self.archetype.types() {
            let ptr = self.archetype.get_dynamic(ty.id(), ty.layout().size(), self.index).unwrap();
            f(ptr.as_ptr(), ty);
        }
    }
}
