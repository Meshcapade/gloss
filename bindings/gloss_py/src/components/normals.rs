...
use gloss_renderer::{components::Normals, scene::Scene};
...
#[pyclass(name = "Normals", module = "gloss.components", unsendable)]
// it has to be unsendable because it does not implement Send: https://pyo3.rs/v0.19.1/class#must-be-send
#[derive(Clone, PyComponent)]
pub struct PyNormals {
    pub inner: Normals,
}
#[pymethods]
impl PyNormals {
    #[new]
    #[pyo3(text_signature = "(array: NDArray[np.float32]) -> Normals")]
    pub fn new(array: PyReadonlyArray2<f32>) -> Self {
        let shape = array.shape();
        assert_eq!(shape[1], 3, "Normals need to be a Nx3 matrix but it has shape {shape:?}");
        Self {
            inner: Normals(DynamicTensorFloat2D::from_dmatrix(&array.as_matrix().into())),
        }
    }
