...
pub fn struct_fields(fields: &syn::Fields) -> (Vec<&syn::Type>, Vec<syn::Member>) {
    match fields {
        syn::Fields::Named(ref fields) => fields.named.iter().map(|f| (&f.ty, syn::Member::Named(f.ident.clone().unwrap()))).unzip(),
...
pub fn member_as_idents(members: &[syn::Member]) -> Vec<Cow<'_, syn::Ident>> {
    members
        ...
        .map(|member| match member {
            ...
            &syn::Member::Unnamed(syn::Index { index, span }) => Cow::Owned(syn::Ident::new(&format!("tuple_field_{}", index), span)),
        })
        .collect()
}
