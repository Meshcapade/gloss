use gloss_hecs::{Entity, PreparedQuery, With, World};
...
fn system_fire_at_closest(world: &mut World) {
    for (id0, (pos0, dmg0, mut kc0)) in &mut world.query::<With<(&Position, &Damage, &mut KillCount), &Health>>() {
        // Find closest:
        ...
        */
        ...
        if hp1.0 > 0 {
            // apply damage
            hp1.0 -= dmg0.0;
            println!("Unit {:?} was damaged by {:?} for {:?} HP", closest, id0, dmg0.0);
            if hp1.0 <= 0 {
