...
use crate::{
    query::{assert_borrow, Fetch, With, Without},
    Archetype, Query, QueryItem,
};

/// A borrow of a [`World`](crate::World) sufficient to execute the query `Q` on
/// a single entity
...
impl<'a, Q: Query> QueryOne<'a, Q> {
    ...
    ///
    /// Panics if called more than once or if it would construct a borrow that
    /// clashes with another pre-existing borrow.
    ...
    pub fn get(&mut self) -> Option<QueryItem<'_, Q>> {
        ...
        // }
        assert!(!self.borrowed, "called QueryOnce::get twice; construct a new query instead");
